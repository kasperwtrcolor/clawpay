...
app.post('/x-webhook', async (req, res) => {
  try {
    ...
    for (const tweet of event.tweet_create_events) {
      ...
      if (!text.includes('@bot_claw')) continue;

      const command = parsePaymentCommand(text);
      if (!command) {
        await rwClient.v2.reply(
          `❌ Invalid command. Try: "@bot_claw send @username $5"`,
          id_str
        );
        continue;
      }
      ...
      if (result.success) {
        await rwClient.v2.reply(
          `✅ @${fromUser} sent $${amount} USDC to @${toHandle}! #CLAWPAY`,
          id_str
        );
      } else {
        await rwClient.v2.reply(
...
app.listen(PORT, () => console.log(`🤖 CLAW Bot running on port ${PORT}`));
